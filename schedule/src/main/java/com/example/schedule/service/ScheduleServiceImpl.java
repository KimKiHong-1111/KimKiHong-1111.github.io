...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    @Transactional
    @Override
    public ScheduleResponseDto updateSchedule(String name, Long pw, String contents) {
        //필수값 검증
    ...
    public void deleteSchedule(String name) {
        //schedule 삭제
        int deleteRow = scheduleRepository.deleteschedule(name);
        //NPE 방지 삭제된 row가 0개라면
        //수정 필요함 pw가 아닌 id로 바껴야함.
        if (deleteRow == 0) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Does not exist name = " + name);
        }
    }
